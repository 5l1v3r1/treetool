...
class flu_filter(virus_filter):
	...
	def filter_geo(self, prune = True):
		...
		reader = csv.DictReader(open(self.path_to_augur+"/source-data/geo_synonyms.tsv"), delimiter='\t')		# list of dicts
		label_to_country = {}
		for line in reader:
		...
		reader = csv.DictReader(open(self.path_to_augur+"/source-data/geo_regions.tsv"), delimiter='\t')		# list of dicts
		country_to_region = {}
		for line in reader:
